...
# Define the TensorBoard callback. It will save the logs to the specified folder. The callback needs to be
# provided to the list of callbacks in the model.fit call.
...
# To get the TensorBoard logging running, you also need to add the callback to the callbacks list.
model.fit(x_train, y_train, epochs=15, validation_data=(x_test, y_test), callbacks=[cp_callback, tb_callback])
